...
namespace csharp_test_client
{
    public partial class mainForm : Form
    {
            ...
            while (IsNetworkThreadRunning)
            {
                ...
                if (recvData != null)
                {
                    ...
                    while (true)
                    {
                        ...
                        // not enought space in PacketBuffer for new packet to be pushed.
                        if (data.Count < 1)
                            break;
                        ...
                        {
                            // packet reading -> BackGroundProcess
        ...
        object FindRoomUserList(Int64 userUniqueId)
        {
        ...
        string GetRoomUserId(Int64 userUniqueId)
        {
            foreach( var user in listBoxRoomUserList.Items)
            {
                var items = user.ToString().Split(":");
                if (items[0].ToInt64() == userUniqueId)
                {
                    return items[1].ToString().TrimStart(' ');
                }    
            }
            return "";
        }
        ...
        {
            object removeItem = FindRoomUserList(userUniqueId);
